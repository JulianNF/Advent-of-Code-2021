...
function lowerAim(amount) {
	aim += amount;
};

function raiseAim(amount) {
	aim -= amount;
};

function moveSubmarine(amount) {
	advance += amount;
	depth += amount * aim;
};
...
console.log(`advance: ${advance}, depth: ${depth} ==> multiple: ${advance * depth}`);
