...
function isLowPoint(point, map) {
    ...
    let valueLeft = 9;
    if (point.x > 0)
        valueLeft = map[point.y][point.x - 1];
    if (value >= valueLeft)
        return false;
    let valueRight = 9;
    const maxX = map[0].length - 1;
    if (point.x < maxX)
        valueRight = map[point.y][point.x + 1];
    if (value >= valueRight)
        return false;
    let valueUp = 9;
    if (point.y > 0)
        valueUp = map[point.y - 1][point.x];
    if (value >= valueUp)
        return false;
    let valueDown = 9;
    const maxY = map.length - 1;
    if (point.y < maxY)
        valueDown = map[point.y + 1][point.x];
    if (value >= valueDown)
        return false;
    return true;
}
function findLowPointsInMap(map) {
    let lowPoints = [];
...
function getAdjacentPoints(point, map) {
    let adjacentPoints = [];
    const maxX = map[0].length - 1;
    if (point.x > 0)
...
function getPointValue(point, map) {
    return map[point.y][point.x];
}
function isPointInPointArray(point, pointArray) {
...
function getPointsInBasin(lowPoint, map) {
    ...
    while (pointsToTest.length > 0) {
        ...
        for (let i = pointsToTest.length - 1; i >= 0; i--) {
            ...
            let adjacentPointsAlreadyInBasin = [];
            let adjacentPointsNotCurrentlyInBasin = [];
...
function findThreeLargestBasins(lowPoints, map) {
    let basinSizes = [];
