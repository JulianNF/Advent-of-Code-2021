...
// });
console.log('visited', riskGraph.reduce((total, vertex) => (vertex.visited === true ? (total += 1) : total), 0), '/', riskGraph.length);
...
function generateNewRiskValue(integerString, incrementBy) {
...
function stepsToEndFrom(startX, startY) {
    return graphWidth - 1 - startX + (graphHeight - 1 - startY);
}
function generateInitialRiskGraph(textRows) {
    let riskGraph = [];
...
function identifyNextVertexToVisit() {
...
function getAdjacentVertexIds(x, y) {
    let verteces = [];
    if (x < graphWidth - 1)
...
function getAdjacentVertecesTo(x, y) {
...
function calculateLowestRiskFromStartToFinish(startVertex, endVertex, graph) {
