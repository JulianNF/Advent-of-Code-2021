...
const testSegmentDataObservations = [
	{
...
function identifyKeyNumbers(observationInput) {
	let keyNumbers = {
		one: [],
		seven: [],
		four: []
	...
	let inputValues = observationInput.split(' ');
	for (let value of inputValues) {
		let valueLength = value.length;
		if (valueLength == 2) keyNumbers.one = value.split('');
		if (valueLength == 3) keyNumbers.seven = value.split('');
		if (valueLength == 4) keyNumbers.four = value.split('');
	}

	return keyNumbers;
}

function identifyKeyPatternsFromUniqueNumbers(keyNumbers) {
	let keyPatterns = {
		rightSide: [],
		top: [],
		armOfFour: []
	...
	keyPatterns.rightSide = keyNumbers.one;

	for (let letter of keyNumbers.seven) {
		if (keyNumbers.one.indexOf(letter) < 0) {
			keyPatterns.top = letter;
			break;
		}
	}

	for (let letter of keyNumbers.four) {
		if (keyNumbers.one.indexOf(letter) < 0) {
			keyPatterns.armOfFour.push(letter);
		}
	}
...
function textHasPattern(textString, patternCharArray) {
	for (let letter of patternCharArray) {
		if (textString.indexOf(letter) < 0) return false;
	}
	return true;
}

function decipherStringToDigit(textString, keyPatterns) {
	switch (textString.length) {
		...
		case 7:
			return 8;
		case 5:
			if (textHasPattern(textString, keyPatterns.armOfFour)) return 5;
			if (textHasPattern(textString, keyPatterns.rightSide)) return 3;
			else return 2;
		case 6:
			if (!textHasPattern(textString, keyPatterns.armOfFour)) return 0;
			if (!textHasPattern(textString, keyPatterns.rightSide)) return 6;
			else return 9;
	}
}

function determineOutputValue(observationOutput, keyPatterns) {
	let digits = [];
	let outputStrings = observationOutput.split(' ');
	for (let outputString of outputStrings) {
		digits.push(decipherStringToDigit(outputString, keyPatterns));
	}
...
function calculateSumOfOutputsForAllObservations(segmentDataObservations) {
	let outputSum = 0;
	for (let observation of segmentDataObservations) {
		let uniqueNumbers = identifyKeyNumbers(observation.input);
		// console.log('\n\nuniqueNumbers:', uniqueNumbers);
		let keyPatterns = identifyKeyPatternsFromUniqueNumbers(uniqueNumbers);
		// console.log('keyPatterns:', keyPatterns);
		
		outputSum += determineOutputValue(observation.output, keyPatterns);
	}
	return outputSum;
}

// console.log('output sum:', calculateSumOfAllOutputValues(testSegmentDataObservations));
console.log('output sum:', calculateSumOfOutputsForAllObservations(segmentDataObservations));

// for (let i = 0; i < 10000; i++) {
// countUniqueValuesInsegmentDataObservationsOutput(segmentDataObservations);
// }
