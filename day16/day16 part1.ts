...
writeFileSync('testOutput.json', JSON.stringify(packet));
...
function literalDataToValue(data: string): number {
...
function splitSubpacketBitsIntoArray(bits: string): string[] {
	let bitArray: string[] = [];
...
// Reviewed - This makes sense:
function getNextEndIndex(bitString: string): number {
...
// Reviewed - This makes sense:
function getNumberOfSubpacketsInCountOperatorPacket(bitString: string): number {
	let numberOfHeaderBits = 3 + 3 + 1; // 3 version bits, 3 type ID bits, 1 length-type ID bit
