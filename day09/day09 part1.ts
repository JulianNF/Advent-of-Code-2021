...
function getPointDepthValues(point: Point, map: number[][]): PointDepthValues {
    let depthValues: PointDepthValues = {
        point: NaN,
        up: NaN,
        down: NaN,
        left: NaN,
    ...
    if (point.y > 0) depthValues.up = map[point.y - 1][point.x];
    if (point.y < maxY) depthValues.down = map[point.y + 1][point.x];
    if (point.x > 0) depthValues.left = map[point.y][point.x - 1];
    if (point.x < maxX) depthValues.right = map[point.y][point.x + 1];

    return depthValues;
}

function isLowPoint(pointValues: PointDepthValues): boolean {
    if (pointValues.point == 0) return true;
    if (pointValues.point == 9) return false;
    if (pointValues.point >= pointValues.up) return false;
    if (pointValues.point >= pointValues.down) return false;
    if (pointValues.point >= pointValues.left) return false;
    if (pointValues.point >= pointValues.right) return false;
    return true;
}

function riskLevelOfPoint(pointValue: number): number {
    return pointValue + 1;
}

function getTotalRiskLevel(map: number[][]): number {
